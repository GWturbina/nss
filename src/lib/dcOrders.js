  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        fancy_color: params.fancyColor || null,
        intensity: params.intensity || null,
        carats: params.carats,
        has_cert: params.hasCert,
        region: params.region,
        buy_mode: params.buyMode,
        is_fraction: params.isFraction || false,
        fraction_count: params.fractionCount || 0,
        total_fractions: params.totalFractions || 0,
        retail_price: params.retailPrice,
        club_price: params.clubPrice,
        savings: params.savings,
        discount_pct: params.discountPct,
        spec_string: params.specString,
        status: 'PAID',
    ...
    // Записать в лог
    await addOrderLog(data.id, 'CREATED', wallet, `Заказ: ${params.specString} — $${params.clubPrice}`)
    await addOrderLog(data.id, 'PAID', wallet, `Оплачено: $${params.clubPrice}`)

    return { ok: true, order: data }
  } catch (e) {
    return { ok: false, error: e.message || 'Ошибка создания заказа' }
  }
}
...
export async function updateOrderStatus(orderId, newStatus, adminWallet, note = '') {
  ...
  try {
    // Проверяем права
    const role = await getAdminRole(adminWallet)
    if (!role) return { ok: false, error: 'Нет прав администратора' }
    if (role === 'operator') return { ok: false, error: 'Оператор не может менять статус' }

    // Проверяем лимит суммы для manager
    if (role === 'manager') {
      const order = await getOrder(orderId)
      if (!order) return { ok: false, error: 'Заказ не найден' }
      const admin = await getAdminInfo(adminWallet)
      if (admin && admin.max_amount > 0 && order.club_price > admin.max_amount) {
        return { ok: false, error: `Сумма $${order.club_price} превышает ваш лимит $${admin.max_amount}` }
      }
    }
    ...
    if (newStatus === 'APPROVED') { updates.approved_by = adminWallet.toLowerCase(); updates.approved_at = now }
    if (newStatus === 'PRODUCTION') { updates.production_at = now }
    if (newStatus === 'READY') { updates.ready_at = now }
    if (newStatus === 'COMPLETED') { updates.completed_by = adminWallet.toLowerCase(); updates.completed_at = now }
    ...
    await addOrderLog(orderId, newStatus, adminWallet, note || `Статус → ${newStatus}`)

    return { ok: true }
  } catch (e) {
    return { ok: false, error: e.message || 'Ошибка обновления' }
  }
}
