...
export default function GemConfigurator() {
  const { wallet, t, addNotification, setTxPending, txPending, nst } = useGameStore()
  ...
  const price = useMemo(() => {
    if (gemType === 'white') {
      return calcWhitePrice(shape, clarity, color, carats, hasCert, region, userNst)
    } else {
      return calcFancyPrice(shape, fancyColor, intensity, clarity, carats, hasCert, region, userNst)
    }
  }, [gemType, shape, clarity, color, fancyColor, intensity, carats, hasCert, region, userNst])
  ...
  return (
    <div className="px-3 mt-2 space-y-2">
      ...
      <div className="p-3 rounded-2xl glass">
        ...
        {price && <div className="mt-1.5 text-[8px] text-slate-600 text-center">
          {t('gcRegionMarkup')}: +{price.regionMarkupPct}%
        </div>}
      </div>
        ...
        <div className="p-3 rounded-2xl border border-gold-400/20"
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-[9px] text-slate-500">
                {carats} ct • {hasCert ? '✅ Сертификат' : 'Без серт.'} • {REGIONS.find(r=>r.id===region)?.name}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-1.5 mb-3">
            <div className="p-2 rounded-xl bg-white/5 text-center">
              <div className="text-[7px] text-slate-600 mb-0.5">{t('gcRetailPrice')}</div>
            ...
            <div className="p-2 rounded-xl bg-gold-400/8 border border-gold-400/15 text-center">
              <div className="text-[7px] text-gold-400/70 mb-0.5">{t('gcClubPrice')}</div>
            ...
            <div className="p-2 rounded-xl bg-emerald-500/8 border border-emerald-500/15 text-center">
              <div className="text-[7px] text-emerald-400/70 mb-0.5">{t('gcSavings')}</div>
              <div className="text-[13px] font-black text-emerald-400">
                −{formatUSD(price.savings)}
              </div>
              <div className="text-[7px] text-emerald-400/60">
                −{price.discountPct}%{price.nstBonus > 0 ? ` (NST +${price.nstBonus}%)` : ''}
              </div>
            </div>
          </div>
